...
impl Mutex {
    ...
    pub const fn new() -> Self {
        Self { owner_thread_handle: AtomicU32::new(svc::INVALID_HANDLE) }
    }
    ...
    pub fn try_lock(&self) -> bool {
        self.owner_thread_handle.compare_exchange(svc::INVALID_HANDLE, get_current_thread_handle(), Acquire, Relaxed).is_ok()
    }
    ...
    pub fn lock(&self) {
        ...
        if let Err(state) = self.owner_thread_handle.compare_exchange(svc::INVALID_HANDLE, current_thread, Acquire, Relaxed) {
        ...
        loop {
            ...
            if state & WAIT_MASK == 0 { 
                ...
                if self.owner_thread_handle.fetch_or(WAIT_MASK, Acquire) == svc::INVALID_HANDLE {
                    ...
                    match self.owner_thread_handle.compare_exchange(svc::INVALID_HANDLE | WAIT_MASK, current_thread, Acquire, Relaxed) {
                        Ok(_) => {
                ...
                if let Err(rc) = unsafe {svc::arbitrate_lock(state  & !WAIT_MASK, &self.owner_thread_handle as *const _ as _, current_thread)} {
    ...
    pub unsafe fn unlock(&self) {
        ...
        if state & WAIT_MASK !=0  {
            if let Err(rc) = unsafe {svc::arbitrate_unlock(&self.owner_thread_handle as *const _  as _)} {
                abort::abort(abort::AbortLevel::Panic(), rc);
            }
        }
    }
