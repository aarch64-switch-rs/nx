...
impl CondVar {
    ...
    fn wait(&self, mutex: &Mutex) {
        if let Err(rc) = unsafe {svc::wait_process_wide_key_atomic(mutex as *const _ as _, self as *const Self as _, get_current_thread_handle(), -1)} {
...
impl RwLock {
    ...
    pub fn try_read(&self) -> bool {
        ...
        return no_write_lock;
    }

    pub unsafe fn read_unlock(&self) {
        self.lock.lock();

        if self.reader_lock_count.fetch_sub(1, Release) == 1  && self.write_waiter_count.load(Acquire) > 0 {
