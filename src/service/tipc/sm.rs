...
impl IUserInterface for UserInterface {
    fn register_client(&mut self, process_id: sf::ProcessId) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 0] (process_id) => ())
    }
    ...
    fn unregister_service(&mut self, name: ServiceName) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 3] (name) => ())
    }
    ...
    fn atmosphere_has_mitm(&mut self, name: ServiceName) -> Result<bool> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65004] (name) => (has: bool))
    }

    fn atmosphere_wait_mitm(&mut self, name: ServiceName) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65005] (name) => ())
    }

    fn atmosphere_declare_future_mitm(&mut self, name: ServiceName) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65006] (name) => ())
    }

    fn atmosphere_clear_future_mitm(&mut self, name: ServiceName) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65007] (name) => ())
    }

    fn atmosphere_has_service(&mut self, name: ServiceName) -> Result<bool> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65100] (name) => (has: bool))
    }

    fn atmosphere_wait_service(&mut self, name: ServiceName) -> Result<()> {
        ipc_tipc_client_send_request_command!([self.session.object_info; 65101] (name) => ())
    }
}
