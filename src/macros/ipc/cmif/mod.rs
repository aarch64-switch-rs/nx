...
macro_rules! nipc_cmif_interface_define_command {
    ($name:ident: ( $( $in_param_name:ident: $in_param_type:ty ),* ) => ( $( $out_param_name:ident: $out_param_type:ty ),* )) => {
        ...
        paste::paste! {
            ...
            #[allow(unused_parens)]
            fn [<$name _impl>](&mut self, mut ctx: &mut $crate::ipc::cmif::server::ServerContext) -> $crate::result::Result<()> {
                ctx.raw_data_walker = $crate::ipc::DataWalker::new(ctx.ctx.in_params.data_offset);
                $( let $in_param_name = <$in_param_type as $crate::ipc::cmif::server::CommandParameter<_>>::after_request_read(&mut ctx)?; )*

                let ( $( $out_param_name ),* ) = self.$name( $( $in_param_name ),* )?;

                ctx.raw_data_walker = $crate::ipc::DataWalker::new(core::ptr::null_mut());
                $( $crate::ipc::cmif::server::CommandParameter::<_>::before_response_write(&$out_param_name, &mut ctx)?; )*
                ...
                $( $crate::ipc::cmif::server::CommandParameter::<_>::after_response_write(&$out_param_name, &mut ctx)?; )*
...
macro_rules! nipc_cmif_interface_make_command_meta {
    ...
    ($name:ident: $id:expr, [($major:expr, $minor:expr, $micro:expr) =>]) => {
        paste::paste! {
            $crate::ipc::cmif::sf::CommandMetadata::new($id, unsafe { core::mem::transmute(Self::[<$name _impl>] as fn(&mut Self, &mut $crate::ipc::cmif::server::ServerContext) -> $crate::result::Result<()>) }, Some($crate::version::Version::new($major, $minor, $micro)), None)
        }
    };
    ($name:ident: $id:expr, [=> ($major:expr, $minor:expr, $micro:expr)]) => {
        paste::paste! {
            $crate::ipc::cmif::sf::CommandMetadata::new($id, unsafe { core::mem::transmute(Self::[<$name _impl>] as fn(&mut Self, &mut $crate::ipc::cmif::server::ServerContext) -> $crate::result::Result<()>) }, None, Some($crate::version::Version::new($major, $minor, $micro)))
        }
    };
    ($name:ident: $id:expr, [($major_a:expr, $minor_a:expr, $micro_a:expr) => ($major_b:expr, $minor_b:expr, $micro_b:expr)]) => {
        paste::paste! {
            $crate::ipc::cmif::sf::CommandMetadata::new($id, unsafe { core::mem::transmute(Self::[<$name _impl>] as fn(&mut Self, &mut $crate::ipc::cmif::server::ServerContext) -> $crate::result::Result<()>) }, Some($crate::version::Version::new($major_a, $minor_a, $micro_a)), Some($crate::version::Version::new($major_b, $minor_b, $micro_b)))
        }
    };
}