...
ipc_sf_define_interface_trait! {
	trait LocationResolverManager {
        open_location_resolver [0, version::VersionInterval::all()]: (storage_id: ncm::StorageId) => (resolver: LocationResolver);
        open_registered_location_resolver [1, version::VersionInterval::all()]: () => (resolver: RegisteredLocationResolver);
    }
}
