...
pub trait IDatabaseService {
    ipc_interface_define_command!(is_updated: (flag: SourceFlag) => (updated: bool));
    ipc_interface_define_command!(is_full: () => (full: bool));
    ipc_interface_define_command!(get_count: (flag: SourceFlag) => (count: u32));
    ipc_interface_define_command!(get_1: (flag: SourceFlag, out_char_infos: sf::OutMapAliasBuffer) => (count: u32));
    ipc_interface_define_command!(build_random: (age: Age, gender: Gender, face_color: FaceColor) => (char_info: CharInfo));
}

pub trait IStaticService {
    ipc_interface_define_command!(get_database_service: (key_code: SpecialKeyCode) => (database_service: mem::Shared<dyn sf::IObject>));
}