...
ipc_sf_define_interface_trait! {
    trait ApplicationDisplay {
        ...
        get_system_display_service [101, version::VersionInterval::all()]: () => (system_display_service: SystemDisplay | session_type!(SystemDisplay) );
        get_manager_display_service [102, version::VersionInterval::all()]: () => (manager_display_service: ManagerDisplay | session_type!(ManagerDisplay) );
...
ipc_sf_define_interface_trait! {
    trait SystemDisplayRoot {
        get_display_service [1, version::VersionInterval::all()]: (mode: DisplayServiceMode) => (display_service: ApplicationDisplay | session_type!(ApplicationDisplay) );
    }
}

ipc_sf_define_interface_trait! {
    trait ManagerDisplayRoot {
        get_display_service [2, version::VersionInterval::all()]: (mode: DisplayServiceMode) => (display_service: ApplicationDisplay | session_type!(ApplicationDisplay) );
    }
}
