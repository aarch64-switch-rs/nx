...
impl<'ctx> ServerContext<'ctx> {
    pub const fn new(ctx: &'ctx mut CommandContext, raw_data_walker: DataWalker, domain_table: Option<mem::Shared<DomainTable>>, new_sessions: &'ctx mut Vec<ServerHolder>) -> Self {
...
pub trait ResponseCommandParameter {
    ...
    fn after_response_write(var: Self, carry_state: Self::CarryState, ctx: &mut ServerContext) -> Result<()>;
}
...
impl<const M: HandleMode> ResponseCommandParameter for sf::Handle<M> {
    ...
    fn after_response_write(_handle: Self, _carry_state: (), _ctx: &mut ServerContext) -> Result<()> {
        Ok(())
    }
}
...
impl RequestCommandParameter<sf::AppletResourceUserId> for sf::AppletResourceUserId {
    fn after_request_read(ctx: &mut ServerContext) -> Result<Self> {
        result_return_unless!(ctx.ctx.object_info.uses_cmif_protocol(), sf::hipc::rc::ResultUnsupportedOperation);

        if ctx.ctx.in_params.send_process_id {
            Ok(sf::AppletResourceUserId::from(ctx.ctx.in_params.process_id, ctx.raw_data_walker.advance_get::<u64>())) 
        }
        else {
            sf::hipc::rc::ResultUnsupportedOperation::make_err()
        }
...
impl<S: super::client::IClientObject> RequestCommandParameter<S> for Arc<Mutex<S>> {
...
impl<S: super::client::IClientObject + ?Sized> RequestCommandParameter<mem::Shared<S>> for mem::Shared<S> {
...
impl<S: ISessionObject + 'static> ResponseCommandParameter for S {
    ...
    fn before_response_write(_session: &Self, ctx: &mut ServerContext) -> Result<u32> {
        if ctx.ctx.object_info.is_domain() {
            let domain_object_id = ctx.domain_table.as_mut().ok_or(rc::ResultDomainNotFound::make())?.lock().allocate_id()?;
    ...
    fn after_response_write(session: Self, carry_state: u32, ctx: &mut ServerContext) -> Result<()> {
        if ctx.ctx.object_info.is_domain() {
            ctx.domain_table.as_mut().ok_or(rc::ResultDomainNotFound::make())?.lock().domains.push(ServerHolder::new_domain_session(0, carry_state, mem::Shared::new(session)))
        } else {
            ctx.new_sessions.push(ServerHolder::new_session(carry_state, mem::Shared::new(session)));
        }
        Ok(())
    }
}
...
impl ServerHolder {
    ...
    pub fn make_new_mitm_session(&self, handle: svc::Handle, forward_handle: svc::Handle, info: sm::mitm::MitmProcessInfo, service_name: sm::ServiceName) -> Result<Self> {
            ...
            #[cfg(feature = "services")]
            {
                ...
                match (self.is_mitm_service, self.handle_type) {
                    (true, WaitHandleType::Server) => {
                        debug_assert!(self.info.owns_handle, "MitM server objects should always own their handles.");
...
impl<S: IMitmService> ISessionObject for MitmQueryService<S> {
    fn try_handle_request_by_id(&mut self, req_id: u32, protocol: CommandProtocol, server_ctx: &mut ServerContext) -> Option<Result<()>> {
        <Self as IMitmQueryServiceServer>::try_handle_request_by_id(self, req_id, protocol, server_ctx)
    }
}
                ...
                if server_info.handle == ctx.object_info.handle {
                    ...
                    let target_server = match is_domain {
                        true => match ctx.object_info.owns_handle {
                            ...
                            false => domain_table.ok_or(rc::ResultDomainNotFound::make())?.lock().find_domain(ctx.object_info.domain_object_id)?
                        },
                    ...
                    let mut command_found = false;
                    {
                        ...
                        if let Some(result) = target_server.lock().try_handle_request_by_id(rq_id, protocol, &mut server_ctx) {
                            command_found = true;
        ...
        match domain_command_type {
            ...
            cmif::DomainCommandType::Close => {
                if !ctx.object_info.owns_handle {
                    domain_table_clone.ok_or(rc::ResultDomainNotFound::make())?.lock().deallocate_domain(ctx.object_info.domain_object_id);
                }
                else {
                    // TODO: Abort? Error?
                }
        ...
        for server_holder in &mut self.server_holders {
            ...
            if server_info.handle == ctx.object_info.handle {
                ...
                let mut command_found = false;
                {
                    ...
                    if let Some(result) = <HipcManager as ISessionObject>::try_handle_request_by_id(&mut hipc_manager, rq_id, CommandProtocol::Cmif, &mut server_ctx) {
                        command_found = true;
                        if let Err(rc) = result {
                    ...
                    WaitHandleType::Server => {
                        ...
                        if server_holder.is_mitm_service {
                            #[cfg(feature = "services")]
                            {
                                ...
                                new_sessions.push(server_holder.make_new_mitm_session(new_handle, session_handle.handle, info, server_holder.service_name)?);
                                sm.detach_client(sf::ProcessId::new())?;
                            }
                        }
                        else {
